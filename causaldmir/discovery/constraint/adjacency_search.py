    ...
    for condition_size in range(0, n - 1):
        ...
        for node_u, node_v in remove_edges:
            if g.is_connected(node_u, node_v):
                ...
                if verbose:
                    print(f'Remove edge between {node_u} and {node_v}.')
        ...
        def get_sep_result(node_u, node_v):
            ...
            def sep_search(sep_result, adj):
                for nodes_z in combinations(adj, condition_size):
                    pval = independence.test(node_u, node_v, nodes_z)
                    if pval >= alpha:
                        if sep_result is None:
                            sep_result = node_u, node_v, set(nodes_z)
                        else:
                            sep_result[2] |= nodes_z
                        sep_set[node_u, node_v] |= nodes_z
                        sep_set[node_v, node_u] |= nodes_z
                return sep_result

            sep_result = sep_search(sep_result, g.get_neighbours(node_u))
            sep_result = sep_search(sep_result, g.get_neighbours(node_v))
