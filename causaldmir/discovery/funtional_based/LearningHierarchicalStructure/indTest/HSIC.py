...
def hsic_gam(X, Y, alph = 0.01):
    ...
    al = mHSIC**2 / varHSIC #k_appr
    bet = varHSIC*n / mHSIC #theta_appr

    #p_appr = 1-gamcdf(Sta, k_appr, theta_appr);
    pval=1-gamma.cdf(testStat,al,bet)
    #print('pval:',pval[0][0])
...
def independent(x, y, alpha = 0.05):
    lens = len(x)
    x1 = x.reshape(lens,1)
    y1 = y.reshape(lens,1)
    testStat, thresh = hsic_gam(x1, y1, alpha)

##    return testStat, thresh

    if testStat < thresh:
        return True
    elif testStat > thresh:
        return False
