...
from cdmir.discovery.score_based.entropy_based.entropy_based_discovery import utils
from cdmir.discovery.score_based.entropy_based.entropy_based_discovery.lbfgsb_scipy import LBFGSBScipy
from cdmir.discovery.score_based.entropy_based.entropy_based_discovery.locally_connected import LocallyConnected
...
def dual_ascent_step(model, X, lambda1, lambda2, rho, alpha, h, rho_max):
    ...
    while rho < rho_max:
        def closure():
            optimizer.zero_grad()
            X_hat = model(X_torch)
            ent_loss = utils.entropy_loss_mentappr(X_hat, X_torch)
            # mse_loss = squared_loss(X_hat, X_torch)
            h_val = model.h_func()
            penalty = 0.5 * rho * h_val * h_val + alpha * h_val
            l2_reg = 0.5 * lambda2 * model.l2_reg()
            l1_reg = lambda1 * model.fc1_l1_reg()
            primal_obj = ent_loss + penalty + l2_reg + l1_reg
            primal_obj.backward()
            return primal_obj
