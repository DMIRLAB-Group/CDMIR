...
def MergeCausalCluster(LearnedClusters, PureClusters, ImpureClusters, AllCausalCluster, GeneralPureClusters,
                       LatentIndex, data, Ora_data, alpha=0.01):
    """
    ...
    for S in LearnedClusters:
        if Utils.GeneralSetContains(S, ImpureClusters):
            L, Flag = OutterEarlyLearning(RemainingVariables, S, data, Ora_data, LatentIndex, PureClusters,
                                          AllCausalCluster, alpha)
            if Flag:
    ...
    while len(LearnedClusters) >= 1:
        ...
        for C2 in LearnedClusters:  # test whether C1 and C2 share one latent parents
            ...
            if Utils.GeneralSetContains(C1, PureClusters) and Utils.GeneralSetContains(C2, PureClusters):
                ...
                print(
                    "something error ! Because C1 or C2 are not in the record list, such as pure/impure cluster, generalpurity cluster...")

            if flag:
                HopetoMerge.append(C2)
                flag = False

        # Only when C1, C2, C3, and Cn share the same early learning, will they be merged and learned early. Otherwise, update them early first, and then merge the clusters
    ...
    return [MergeCluster, EarlyLearningImpureClusters, EarlyLearningRemoveClusters, IntroduceLatent_PureClusters,
            RemainingVariables], PureClusters, ImpureClusters, AllCausalCluster, GeneralPureClusters, LatentIndex
