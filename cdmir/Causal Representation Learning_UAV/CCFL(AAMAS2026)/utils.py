...
def generate_points(ptBlu=[1,0], maxdist=12, num_env=20, num_barrier=5, dis=2, epoch=0):
    ...
    #     point = [radius * math.cos(ptBlu[0] + 2 * math.pi * i / num_env),
    #              ptBlu[1] + radius * math.sin(2 * math.pi * i / num_env)]
    #     point.append(-2)
...
def Cycle_position(num_env, radius, num_barrier=4, ptBlu=[0, 9]):
    ...
    while True:  # create a potentailly infinite loop! pay attention!不断生成红色点，直到满足特定的条件才会结束循环
        if pointcounter < num_env + num_barrier:  # set the number of point you want to add (in this case 20)pointcounter是否小于指定的点的总数，其中num_env是要生成的红色点的数量，num_barrier是障碍物的数量。这个条件确保不会生成太多的红色点
            ...
            break
    if num_barrier > 0:
        barrier_position = [list(t) for t in zip(xredptlist[-num_barrier:], yredptlist[-num_barrier:], [-int(np.random.uniform(0, 70)) / 10 for _ in range(num_barrier)])]
...
#   linche 圆形生成
#---------------------------------------------------#
# def Cycle_position(num_env, radius, num_barrier, ptBlu=[0, 9]):
#     assert num_env % 2 == 0
#     # barrier_position = None
#
#     position = []
...
#     for i in range(num_barrier):
#         point = [0.5 * np.random.uniform(40, 140) / 10 * math.cos(ptBlu[0] + 2 * math.pi * i / num_barrier), ptBlu[1] + 0.5 * np.random.uniform(40, 140) / 10 * math.sin(2 * math.pi * i / num_barrier)]
#         # 生成一个介于-3到-5之间的随机小数，并保留两位小数
#         random_number = round(random.uniform(-6, -3), 2)
#
#
#         point.append(random_number)
#         # point.append(i * 2 * np.pi / num_barrier + np.pi)
#         barrier_position.append(point)
#
#     print(len(point))
...
# def Cycle_position11(num_env, radius, ptBlu=[0, 9]):
#     assert num_env % 2 == 0
#     barrier_position = None
#     position = []
#     for i in range(num_env):
