...
class Environment():
    ...
    def __init__(self, index, max_timesteps):
        ...
        self.client = airsim.MultirotorClient("127.0.0.1", port=41452)
        self.client.confirmConnection()
        self.index = index
        self.max_timesteps = max_timesteps
        node_name = 'BWorldEnv_' + str(index)
        ...
        cmd_vel_topic = 'airsim_node2/Drone_' + str(
            index) + '/vel_cmd_body_frame'  # 用于发布无人机速度控制指令的ROS话题（Topic）名称。该话题的名称包括无人机的索引（index）以及控制命令的类型（vel_cmd_body_frame）
        self.cmd_vel = rp.Publisher(cmd_vel_topic, VelCmd,
                                    queue_size=10)  # ROS发布者对象，它使用指定的话题名称（cmd_vel_topic）来发布类型为 VelCmd 的消息。消息会被发布到该话题，并可以被其他ROS节点订阅

        dist_topic = '/airsim_node2/Drone_' + str(index) + '/distance/Distance_' + str(
            index)  # 用于订阅距离数据的ROS话题名称。该话题的名称包括无人机的索引（index）和距离数据的类型（Distance_）
        self.dist_sub = rp.Subscriber(dist_topic, Range,
                                      self.dist_callback)  # 创建了一个 ROS 订阅者（Subscriber）对象，并将其与指定的 ROS 话题（Topic）以及消息类型（Range）以及回调函数（self.dist_callback）关联起来

        o_gps_topic = '/airsim_node2/origin_geo_point'  # 用于订阅原点地理位置信息的ROS话题名称
        self.o_gps_sub = rp.Subscriber(o_gps_topic, GPSYaw, self.o_gps_callback)

        m_gps_topic = '/airsim_node2/Drone_' + str(index) + '/gps/gps'  # 用于订阅无人机GPS位置信息的ROS话题名称
    ...
    def drones_terminal(self):  # 终止无人机，悬停或复位无人机
        ...
        if self.reset_flag:  # 于在实验或任务结束时安全地将无人机控制状态归零。
            ...
            [Pose.position.x_val, Pose.position.y_val,
             Pose.position.z_val] = pose  # 将 pose 列表中的值分别赋给 Pose 对象的 position 属性，从而设置了无人机的位置。
            qtn = tf.transformations.quaternion_from_euler(0, 0, 0,
                                                           'rxyz')  # 使用 tf.transformations 库将欧拉角转换为四元数 qtn，表示无人机的姿态。
            [Pose.orientation.x_val, Pose.orientation.y_val, Pose.orientation.z_val,
             Pose.orientation.w_val] = qtn  # 将计算得到的四元数 qtn 的值分别赋给 Pose 对象的 orientation 属性，从而设置了无人机的姿态
            self.client.simSetVehiclePose(vehicle_name="Drone_" + str(self.index), ignore_collision=True,
                                          pose=Pose)  # 过调用 self.client 客户端对象的 simSetVehiclePose 方法来设置无人机的姿态和位置。这相当于将无人机复位到指定的位置和姿态。
            rp.sleep(0.1)
    ...
    def reset_pose(self, init_pos, target_pos):  # 将无人机的姿态重置为指定的姿态，通常在任务开始时使用
        assert len(target_pos) == 3 or len(
            target_pos) == 4  # 检查 target_pos 的长度是否为3或4。如果不满足这个条件，将引发 AssertionError 异常。这是一种用于确保输入数据有效性的方式
        Pose = airsim.Pose()
        pose = list(np.array(target_pos[:3]) - np.array(
            init_pos))  # 计算出无人机从 init_pos 到 target_pos 的位移，将其存储在 pose 变量中。这个位移是一个三维向量，表示无人机需要移动的距离。
        [Pose.position.x_val, Pose.position.y_val,
         Pose.position.z_val] = pose  # 将计算得到的位移分别赋给 Pose 对象的 position 属性，从而设置了无人机的位置。这相当于将无人机的位置设置为 init_pos 和 target_pos 之间的相对位移
        ...
        if len(target_pos) == 3:  # 确定目标位置 target_pos 是否包含了旋转信息。如果长度为 3，表示目标位置没有旋转信息，只包含 x、y 和 z 坐标。如果长度不等于 3，那么就假定目标位置包含了旋转信息
            ...
            qtn = tf.transformations.quaternion_from_euler(0, 0, 1 * np.pi,
                                                           'rxyz')  # 0, 0：这是旋转的 roll 和 pitch 角度,yaw 角度通过 np.random.uniform 从 0 到 2π（360 度）之间的随机值生
        else:  # 如果 target_pos 的长度不等于 3,表示目标位置包含了旋转信息
            qtn = tf.transformations.quaternion_from_euler(0, 0, target_pos[3],
                                                           'rxyz')  # 四元数qtn可以用于将无人机的方向设置为特定的航向角度，以确保它朝向所需的方向
        [Pose.orientation.x_val, Pose.orientation.y_val, Pose.orientation.z_val,
         Pose.orientation.w_val] = qtn  # 将计算得到的四元数 qtn 的值分别赋给 Pose 对象的 orientation 属性，从而设置了无人机的姿态
        self.client.simSetVehiclePose(vehicle_name="Drone_" + str(self.index), ignore_collision=True,
                                      pose=Pose)  # 将无人机的位置和姿态设置为 Pose 对象中指定的位置和姿态
        rp.sleep(1)
        self.client.moveToZAsync(-target_pos[2], velocity=2, vehicle_name="Drone_" + str(
            self.index)).join()  # 将无人机移动到指定的高度。它传递了目标高度 -target_pos[2] 以及速度参数。join() 方法用于等待无人机到达目标高度后再继续执行后续操作
        rp.sleep(0.5)
        ...
        if self.index == 0:
            if num_barrier > 0:
                for i in range(num_barrier):
                    ...
                    [Pose.position.x_val, Pose.position.y_val,
                     Pose.position.z_val] = pose  # 将之前获取的 pose 列表中的 x、y 和 z 坐标分别分配给 Pose 对象的 position 属性，从而设置了障碍物的新位置
    ...
    def get_position(self):  # 获取无人机的位置信息，并将其转换为XY坐标
        ...
        x, y, z = to_xy_3(m_lat, m_lon, m_alt, o_lat, o_lon,
                          o_alt)  # 传入了原点和无人机的经纬度信息，然后计算出了无人机相对于原点的XY坐标和海拔高度，并将其分别赋值给 x、y 和 z。
        return x, y, z

    def get_goal_position(self):
        pos = self.client.simGetGroundTruthKinematics(vehicle_name="Drone_" + str(self.index))
        x = pos.position.x_val
        y = pos.position.y_val
        z = -pos.position.z_val
        return x, y, z
    ...
    def get_local_goal_and_speed(self):  # 获取局部目标和速度信息，涉及到无人机的姿态。
        ...
        [roll, pitch, yaw] = tf.transformations.euler_from_quaternion(
            [Quaternious.x_val, Quaternious.y_val, Quaternious.z_val,
             Quaternious.w_val])  # 将四元数转换为欧拉角，分别获取了滚转角（roll）、俯仰角（pitch）和偏航角（yaw）
        ...
        local_goal = global2body(roll, pitch, yaw, GT_goal,
                                 pos)  # 这行代码调用了一个函数 global2body，该函数用来将全局坐标系中的目标位置 GT_goal 转换为相对于无人机当前位置 pos 的局部目标位置 local_goal。转换中考虑了无人机的姿态（滚转、俯仰和偏航角）。
        v_xyz = np.array(
            [state.kinematics_estimated.linear_velocity.x_val, state.kinematics_estimated.linear_velocity.y_val,
             state.kinematics_estimated.linear_velocity.z_val])  # 获取了无人机的线速度（velocity）信息，包括在局部坐标系中的 x、y 和 z 方向的速度分量，并将其转换为 NumPy 数组 v_xyz。
    ...
    def get_crash_state(self):  # 检查无人机是否发生碰撞
        crash_info = self.client.simGetCollisionInfo(
            vehicle_name="Drone_" + str(self.index))  # 获取与指定无人机相关的碰撞信息。碰撞信息包括了是否发生碰撞以及碰撞的详细信息
    ...
    def get_image(self, noise_std=0):  # 获取无人机的深度图像信息，可能包括噪声
        responses = self.client.simGetImages(
            [airsim.ImageRequest("camera_" + str(self.index), airsim.ImageType.DepthPerspective, True, False)],
            vehicle_name="Drone_" + str(self.index))  # 获取无人机的深度图像信息
        ...
        depth_img_in_meters = airsim.list_to_2d_float_array(response.image_data_float, response.width,
                                                            response.height)  # 图像数据（response.image_data_float）重新排列成二维数组，以正确的宽度和高度表示深度图像。深度图像以浮点数表示，每个像素的值代表距离（通常以米为单位）
        ...
        img = depth_img_in_meters
        img = np.clip(img, a_min=0.2,
                      a_max=20)  # 裁剪深度图像的像素值，确保图像中的值位于指定范围 [0.2, 20] 内。这是因为深度图像通常包含从相机到物体的距离，所以将不合理的距离值限制在合理范围内
    ...
    def generate_goal_point(self, goal):  # 生成目标点，并计算到目标点的距离
        ...
        self.distance = np.sqrt((local_goal[0]) ** 2 + (local_goal[1]) ** 2 + (
        local_goal[2]) ** 2)  # 计算了局部坐标差值的欧氏距离，即无人机当前位置到目标点的距离。这个距离被存储在类的成员变量 self.distance 中
    ...
    def get_reward_and_terminate(self, t, img):  # 计算奖励和终止条件，例如是否到达目标或发生碰撞。
        ...
        self.pre_distance = copy.deepcopy(
            self.distance)  # 存储上一个时间步的距离值，以便在计算奖励或执行其他操作时能够与当前时间步的距离值进行比较，以确定无人机是否靠近或远离目标点，从而影响奖励的计算
        ...
        r_c = w_c * max(d_safe - d_obs,
                        0)  # 计算了基于碰撞的惩罚，如果无人机离障碍物太近（小于 d_safe），则会受到惩罚。奖励值等于 d_safe - d_obs 的最大值乘以权重 w_c。
        result = 0
    ...
    def fly_direction(self, goal):
        self.client.moveToPositionAsync(goal[0], goal[1], -goal[2], velocity=2,
                                        vehicle_name="Drone_" + str(self.index)).join()
    ...
    def plot_trajecy(self, init_pose, path, color_rgba, render_plot):  # 绘制无人机的轨迹
        ...
        if render_plot:
            self.client.simPlotLineList(self.plot_last_pos + plot_v_start, color_rgba=color_rgba[self.index],
                                        thickness=4, is_persistent=True)
        self.plot_last_pos = plot_v_start
        return [x_now, y_now, z_now], path

    def plot_point(self, goal_list, color_rgba):  #画点
    # def plot_point(self, goal_list):  # 画点
        [x_goal, y_goal, z_goal] = goal_list
        plot_goal_list = [airsim.Vector3r(x_goal, y_goal, -z_goal)]
        # self.client.simPlotPoints(plot_goal_list, color_rgba=color_rgba[self.index], size = 60.0, is_persistent = True)
        self.client.simPlotPoints(plot_goal_list, color_rgba, size=5.0, is_persistent=True)
