...
class Actor(nn.Module):
    ...
    def forward(self, x, goal, speed, compute_pi=True, compute_log_pi=True, detach=False):
        a = self.encoder(x, goal, speed, detach=detach)
...
class Critic(nn.Module):
    ...
    def __init__(
        ...
        self.q1 = QFunction(self.encoder.feature_dim, action_shape)
        self.q2 = QFunction(self.encoder.feature_dim, action_shape)
    ...
    def forward(self, x, goal, speed, action, detach=False):
        ...
        v = self.encoder(x, goal, speed, detach=detach)
...
class SAC_Ae_Caps(object):
    ...
    def update_decoder(self, obs, goal, speed, target_obs, target_goal, target_speed):
        h = self.critic.encoder(obs, goal, speed)

        rec_obs, rec_goal, rec_speed = self.decoder(h)
        rec_loss = F.mse_loss(target_obs, rec_obs) + F.mse_loss(target_goal, rec_goal) + F.mse_loss(target_speed, rec_speed)
    ...
    def update(self, batch_size):
        ...
        if self.lam_s > 0:
            b_state_tensor = self.decoder(self.actor.encoder(state_tensor))
        ...
        if self.decoder is not None and self.total_it % self.decoder_update_freq == 0:
            self.update_decoder(state_tensor, goal_tensor, speed_tensor, state_tensor, goal_tensor, speed_tensor)
