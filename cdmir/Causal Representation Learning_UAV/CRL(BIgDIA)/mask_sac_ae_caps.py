...
class Actor(nn.Module): #用于执行动作选择的，通常在强化学习中用作策略网络
    ...
    def forward(self, x, goal, speed, compute_pi=True, compute_log_pi=True,detach=False):
        ...
        feature_one = torch.cat((feature_one, goal, speed), dim=-1)  # 将编码器的输出 a、目标 goal 和速度 speed 沿着最后一个维度（dim=-1 表示最后一个维度）拼接在一起。这将把这些信息合并到一起，以供后续神经网络层使用
        mlp_one = self.trunk_one(feature_one)
        ...
        return mu, pi, log_pi, log_std, percentage_zeros_mask_one, percentage_zeros_mask_two
...
class SAC_Ae_Caps(object):
    ...
    def generate_action(self, env, state_list): #根据输入的环境索引和状态信息生成相应的动作
        if env.index == 0:
            ...
            if self.mode == 'train':
                mu, pi, _, _, percentage_zeros_mask_one, percentage_zeros_mask_two = self.actor(state_tensor, goal_tensor, speed_tensor, compute_log_pi=False)
                ...
                mu, _, _, _, _, _ = self.actor(state_tensor, goal_tensor, speed_tensor, compute_pi=False, compute_log_pi=False)
    ...
    def update_critic(self, state_tensor, goal_tensor, speed_tensor, action, reward,
        ...
        with torch.no_grad():
            _, policy_action, log_pi, _, _, _ = self.actor(n_state_tensor, n_goal_tensor, n_speed_tensor)
    ...
    def update_actor_and_alpha(self, state_tensor, goal_tensor, speed_tensor,
        ...
        mu, pi, log_pi, _, _, _ = self.actor(state_tensor, goal_tensor, speed_tensor, detach=True)    #从Actor网络中获取当前状态下的均值、采样的动作、以及动作的对数概率
        ...
        if self.lam_a > 0:  #检查正则化项的权重是否大于零。如果lam_a大于零，表示开启了正则化项
            mu_nxt, _, _, _, _, _ = self.actor(n_state_tensor, n_goal_tensor, n_speed_tensor, detach=True)    #到下一个状态的动作均值
            actor_loss += self.lam_a * torch.sum(((mu_nxt - mu)**2) / 2) / mu.shape[0]  #这个正则化项衡量了当前状态的动作均值（mu）与下一个状态的动作均值（mu_nxt）之间的差异
        if self.lam_s > 0:
            mu_bar, _, _, _, _, _ = self.actor(b_state_tensor, b_goal_tensor, b_speed_tensor, detach=True)
