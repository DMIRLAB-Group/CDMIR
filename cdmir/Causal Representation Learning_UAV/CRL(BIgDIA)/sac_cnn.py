...
class Actor(nn.Module):
    ...
    def forward(self, x, goal, speed, compute_pi=True, compute_log_pi=True,detach=False):
        a = self.encoder(x, detach=detach)
...
class Critic(nn.Module):
    ...
    def __init__(
        self, obs_shape, hidden_dim, action_shape,
...
class SAC_CNN(object):
    ...
    def __init__(
        ...
        if env.index == 0:
            ...
            self.replayer = self.replayer = utils.ReplayBuffer(self.replayer_buffer)
            self.action_bound = [[0, 1.], [-1., 1.], [-1., 1.]]

            self.total_it = 0

            self.actor = Actor(
            obs_shape, hidden_dim,
            encoder_feature_dim, actor_log_std_min, actor_log_std_max,
            num_layers, num_filters
            ).to(device)

            self.critic = Critic(
                obs_shape, hidden_dim, action_shape,
            ...
            [self.log_alpha], lr=lr, betas=(alpha_beta, 0.999)
            )
    ...
    def update_actor_and_alpha(self, state_tensor, goal_tensor, speed_tensor):
        ...
        _, pi, log_pi, log_std = self.actor(state_tensor, goal_tensor, speed_tensor, detach=True)
